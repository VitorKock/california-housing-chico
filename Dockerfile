...
# and `pip install california` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "california", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n california -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "california", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "california", "YOUR_CONSOLE_SCRIPT"]
